...
namespace edge_matching_puzzle
{
    class emp_situation_binary_dumper
    {
      public:
        /**
         * Constructor
         * @param p_name name of file to dump
         * @param p_FSM_info puzzle characteristics
         * @param p_generator strategy generator for position list
         * @param p_solution_dump true if we are dumping solutions false if we
         * are dumping intermediate solutions
         */
        inline
        emp_situation_binary_dumper(const std::string & p_name
        ...
        inline
        ~emp_situation_binary_dumper();

        inline
        void dump(const emp_FSM_situation & p_situation);

        inline
        void dump(const uint64_t & p_total_number);

        template <typename T>
        void dump(const quicky_utils::quicky_bitfield<T> & p_bitfield
    ...
    //----------------------------------------------------------------------------
    template <typename T>
    void emp_situation_binary_dumper::dump(const quicky_utils::quicky_bitfield<T> & p_bitfield
                                          ,const uint64_t & p_total_number
                                          )
    {
