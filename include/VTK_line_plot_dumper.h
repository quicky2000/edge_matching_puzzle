...
namespace edge_matching_puzzle
{
    ...
    {
        m_vtk_line_plot_file.open(p_file_name.c_str());
        if(!m_vtk_line_plot_file.is_open())
        {
            throw quicky_exception::quicky_logic_exception(R"(Unable to create VTK line_plot file ")" + p_file_name +R"(")", __LINE__, __FILE__);
        }
        m_vtk_line_plot_file << "line_plot" << std::endl;
        m_vtk_line_plot_file << p_tile << " " << p_X_name << " " << p_Y_name << " " << p_serie_dim << " " << p_nb_series << std::endl;

        if(p_series_name.size() != p_nb_series)
        {
            throw quicky_exception::quicky_logic_exception("Number of serie name (" + std::to_string(p_series_name.size()) + ") not coherent with serie number(" + std::to_string(p_nb_series) +")", __LINE__, __FILE__);
        }

        for(auto l_iter = p_series_name.begin();l_iter != p_series_name.end() - 1; ++l_iter)
        {
            m_vtk_line_plot_file << *l_iter << " ";
        }
        m_vtk_line_plot_file << *p_series_name.rbegin() << std::endl;
    }

    //-------------------------------------------------------------------------
    VTK_line_plot_dumper::~VTK_line_plot_dumper()
    {
        if(m_vtk_line_plot_file.is_open())
        {
    ...
    //-------------------------------------------------------------------------
    void
    VTK_line_plot_dumper::dump_serie(const std::vector<unsigned int> & p_serie)
    {
    ...
    //-------------------------------------------------------------------------
    void
    VTK_line_plot_dumper::dump_value(unsigned int p_value)
    {
        ...
        m_vtk_line_plot_file << p_value << " ";
        ++m_serie_value_counter;
    }

    //-------------------------------------------------------------------------
    void
    VTK_line_plot_dumper::close_serie()
    {
        if(m_serie_value_counter != m_serie_dim)
        {
            assert(m_serie_counter < m_series_name.size());
