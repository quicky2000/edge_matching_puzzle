...
namespace edge_matching_puzzle
{
    class emp_piece
    {
      ...
      public:
        ...
        inline
        emp_piece(const emp_types::t_piece_id & p_id
        ...
        inline
        const emp_types::t_color_id & get_color(const emp_types::t_orientation & p_border)const;

        inline
        const emp_types::t_color_id & get_color(const emp_types::t_orientation & p_border
                                               ,const emp_types::t_orientation & p_orientation
                                               )const;

        inline
        const emp_types::t_piece_id & get_id()const;

        inline
        const t_auto_similarity & get_auto_similarity()const;

        inline static
        const std::string & auto_similarity2string(const t_auto_similarity & p_similarity);

        /**
         * Return true if two pieces are identical ( even if rotation is needed
         **/
        inline
        bool operator==(const emp_piece & p_piece)const;

        /**
         * Return true if pieces are identical with second piece rotated as indicated in orientation parameter
         **/
        inline
        bool compare_to(const emp_piece & p_piece
                       ,const emp_types::t_orientation & p_orient
                       )const;
      ...
      protected:
        ...
        inline
        const std::array<emp_types::t_color_id,4> & get_colors()const;
    ...
    //----------------------------------------------------------------------------
    inline
    std::ostream & operator<<(std::ostream & p_stream,const edge_matching_puzzle::emp_piece & p_piece)
    {
    ...
    //------------------------------------------------------------------------
    const emp_piece::t_auto_similarity & emp_piece::get_auto_similarity()const
    {
    ...
    //----------------------------------------------------------------------------
    emp_types::t_kind emp_piece::get_kind()const
    {
        return m_kind;
    }
    ...
    //--------------------------------------------------------------------------
    const emp_types::t_piece_id & emp_piece::get_id()const
    {
        return m_id;
    }

    //--------------------------------------------------------------------------
    const std::array<emp_types::t_color_id,4> & emp_piece::get_colors()const
    {
        return m_colours;
    }
