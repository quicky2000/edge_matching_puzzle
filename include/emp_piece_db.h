...
namespace edge_matching_puzzle
{
    class emp_piece_db
    {
      public:
        inline
        emp_piece_db(const std::vector<emp_piece> & p_pieces
        ...
        **/
        ...
        /**
         Search pieces which specified kind ( corner, border, center ) and
         matching constraint
        **/
        inline
        void get_pieces(const emp_types::t_kind & p_kind
      ...
      private:
        ...
        **/
        static inline
        unsigned int compute_nb_bits(unsigned int p_value);
        ...
        **/
        inline
        void compute_binary_constraints(const emp_piece & p_piece);
        ...
        **/
        inline
        void compute_constraints(const emp_piece & p_piece);

        static inline
        void print_list(const std::string & p_name
                       ,const std::set<unsigned int> & p_list
                       );

        static inline
        void print_auto_similarities(const emp_piece::t_auto_similarity & p_similarity
                                     ,const std::multimap<emp_piece::t_auto_similarity
                                     ,emp_types::t_piece_id> & p_auto_similarities
                                     );

        typedef std::map<emp_types::t_color_id,std::set<emp_types::t_oriented_piece> > t_color2oriented_pieces;

        static inline
        void store_color2piece(t_color2oriented_pieces& p_color2pieces
                              ,const emp_types::t_color_id & p_color_id
                              ,const emp_types::t_oriented_piece & p_piece
                              );

        inline
        void record_identical_pieces(const emp_types::t_piece_id & p_id1
    ...
    ,m_constraint_db(new t_constraint_db*[3])
    {
       ...
       for(auto l_iter: m_border2center_colors_nb)
       {
           ...
           std::cout << "Border2center color " << l_iter.first << " is present on " << l_iter.second
                     << " border pieces\t" << l_remaining << "C" << l_iter.second << " = " << l_combi << std::endl;
